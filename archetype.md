...
你可以把这 12 个定义作为一个 JSON Object 喂给 Gemini 的 System Prompt，让它根据用户的 100 条数据计算出一个 `Match Score`，然后选出最高分的那个作为用户的 **Archetype**。
...
```json
{
  "archetypes": [
    {
      "name": "文艺复兴守门员",
      "criteria": "High percentage of pre-1990 content, preference for classics/black-and-white."
    },
    // ... others
  ]
}
